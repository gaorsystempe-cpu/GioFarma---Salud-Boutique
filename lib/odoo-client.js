...
class OdooClient {
  constructor() {
    // Limpiar URL para evitar // duplicadas
  ...
  async connect() {
    ...
    if (!this.url || !this.db || !this.password) {
      console.error('Faltan variables de entorno para Odoo:', { 
        url: !!this.url, 
        db: !!this.db, 
        user: !!this.username,
        pass: !!this.password 
      });
      throw new Error('Configuración de Odoo incompleta. Verifica VITE_ODOO_URL, VITE_ODOO_DB, VITE_ODOO_USERNAME y VITE_ODOO_API_KEY en Vercel.');
    }
    ...
    return new Promise((resolve, reject) => {
      common.methodCall('authenticate', [this.db, this.username, this.password, {}], (error, value) => {
        if (error) return reject(new Error(`Odoo XML-RPC Auth Error: ${error.message}`));
        if (!value) return reject(new Error('Autenticación fallida en Odoo. Revisa el usuario y la API Key.'));
        this.uid = value;
        resolve(value);
      });
    });
